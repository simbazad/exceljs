...
class Worksheet {
  ...
  async _parseRows(model,looseParsing = false) {
    this._rows = [];
    this._rawRows = [];
    let _backoffCounter = 0;
    for(let i = 0; i < model.rows.length; i++) {
        if(looseParsing){
          let _currentRow = [];
          for(let j = 0; j < model.rows[i].cells.length; j++) {
            _currentRow.push(model.rows[i].cells[j].value);
          }
          this._rawRows.push(_currentRow);
          continue;
        }
        if(_backoffCounter === 10000) {
          await utils.sleep(0);// let gc do its job
          _backoffCounter = 0;
        }
        _backoffCounter++;
        const row = new Row(this, model.rows[i].number);
        this._rows[row.number - 1] = row;
        await row.setModel(model.rows[i]);
    }
  }
  ...
  async setModel(value,looseParsing = false) {
    this.name = value.name;
    this._columns = Column.fromModel(this, value.cols);
    await this._parseRows(value,looseParsing);
    if(looseParsing){
        return;
    }

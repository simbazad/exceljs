...
const {ZipReader , BlobReader , Uint8ArrayReader , TextWriter , Uint8ArrayWriter , TransformStream , GenericTransformStream} = zipJS;
...
class XLSX {
  ...
  async load(data, options) {
    ...
    if (options && options.password) {
      buffer = await officeCrypto.decrypt(Buffer.from(buffer), {password: options.password});
    }

    //let zip = await JSZip.loadAsync(buffer);

    let zip = new ZipReader(
    ...
    for (const entry of entries) {
      ...
      await utils.sleep(0);//let the GC do its thing
      if (!entry.directory) {
        ...
        if (
          ...
          stream.write(Buffer.from(await entry.getData(new Uint8ArrayWriter())));
        } else {
          let content;
    ...
    // apply model
    await this.workbook.setModel(model,options.looseParsing)
